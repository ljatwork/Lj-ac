...
class BugDetailsView: UIView {
    ...
    func setupLocation() -> ElementsView {
        ...
        locationView.detailLabel.text = bug.location.capitalized
        return locationView
    }
    ...
    func setupRarityView() -> ElementsView {
        ...
        rarityView.detailLabel.text = bug.rarity.rawValue.capitalized
        return rarityView
    }
    ...
    func setupNookPriceView() -> ElementsView {
        ...
        rarityView.detailLabel.text = "\(bug.nookPrice.description) \(L10n.bells)"
        return rarityView
    }
    
    func setupCjPriceView() -> ElementsView {
        let rarityView = ElementsView(frame: .zero)
        rarityView.titleLabel.text = L10n.flickPrice
        rarityView.detailLabel.text = "\(bug.flickPrice?.description ?? "") \(L10n.bells)"
        return rarityView
    }
    ...
    func setupTankSizeView() -> ElementsView {
        ...
        rarityView.detailLabel.text = "\(bug.tankWidth) x \(bug.tankLength)"
        return rarityView
    }
    ...
    func setupCatchPhraseContentView() {
        ...
        var catchphrases = ""
        for catchphrase in bug.catchphrases {
...
extension BugDetailsView {
    ...
    func setupNorthAvailabilityView() {
        ...
        var availabilities = ""
        for item in bug.north.availabilityArray {
...
extension BugDetailsView {
    ...
    func setupSouthAvailabilityView()  {
        ...
        availabilityView.titleLabel.text = L10n.availability
        var availabilities = ""
        for item in bug.south.availabilityArray {
            availabilities.append("\(item.months) : \(item.time ?? ""); ")
        }
        availabilityView.detailLabel.text = availabilities
        availabilityView.translatesAutoresizingMaskIntoConstraints = false
